...
func ClientIntercept(ctx context.Context, req, rsp interface{}, ceps []ClientInterceptor, invoke Invoker) error {
	// 如果没有拦截器，直接调用
	if len(ceps) == 0 {
		return invoke(ctx, req, rsp)
	}

	return ceps[0](ctx, req, rsp, getInvoker(0, ceps, invoke))
}
